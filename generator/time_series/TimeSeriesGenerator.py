...
class TimeSeriesGenerator(SyntheticGenerator):
    ...
    dataset_timestamps : list[int]
        The data timestamps for time series values.
    """
    # Class constants
    ...
    def generate(self, num_points: int,
        ...
        stochastic_process : ["AR", "MA"]
             A string either "AR" or "MA" identifying the process from which the
             time series is generated.

        process_params : list[float]
            A list of the params defining the stochastic process using the lag
            polynomial representation including the zero lag. In case or "AR" or
            "MA" the first parameter is the order, the following are the
        ...
        noise : list[str]
            List of noise types for each dimension. Possible noise types are
            "None", "Gaussian" and "custom". The dimension of the list must be
            equal to the number of dimensions of the dataset. Moreover, at index
            `i` there is the noise for dimension `i`. NOTE that this has nothing
            to deal with the noise of the time series. It is an additional noise
            to the dimension. For instance, noise we can have for sensors and
            other.

        custom_noise : list[Callable], default=None
            A list with dimensions equal to the number of dimensions for which
            noise is "custom". The element at index `i` represents the
            implementation of the (i+1)th custom noise. E.g., if we have noise =
            ["Gaussian", "custom", "Gaussian", "custom"]. Then the element
            custom_noise[0] for custom_noise is the implementation of noise[1]
            while custom_noise[1] is the implementation of noise[3].

        verbose : bool, default=True
            If true, detailed prints will be performed. Otherwise, concise
            prints will be done.

        sample_freq_seconds : float, default=1.0
            The number of samples per second, namely the sampling frequency.

        trend : list[bool], default=None
            If true at index `i`, dimension `i` has a trend.

        seasonality : list[bool], default=None
            If true at index `i`, dimension `i` has a seasonality.

        trend_func : list[Trend], default=None
            List of dimension equal to number of True values in trend. The Trend
            at trend_func[i] represents the trend of the (i+1)th True value in
            trend.

        seasonality_func : list[Seasonality], default=None
            List of dimension equal to the number of True values in seasonality.
            The Seasonality at seasonality_func[i] represents the seasonality of
            the (i+1)th True value in seasonality.

        columns_names : list[str], default=None
            List of the column names to use for the time series. If given,
            len(columns_names) must be identical to dim.

        start_timestamp : float, default=datetime.datetime.now().timestamp()
            The timestamp at which to start the time series.

        Returns
        -------
        generator : TimeSeriesGenerator
            The generator itself such that subsequent calls can be concatenated.
        ...
        for point in range(num_points):
            for dim in range(dimensions):
                ...
                if trend is not None and trend[dim]:
                    ...
                    trend_value += trend_func[idx].compute_trend_value(elapsed_seconds)

                if seasonality is not None and seasonality[dim]:
                    idx = sum(seasonality[0:dim + 1]) - 1
                    seasonal_value += seasonality_func[idx].compute_seasonal_value(elapsed_seconds)

                if dimensions > 1:
                    dataset[point, dim, 0] += noise_value + trend_value + seasonal_value
                else:
    ...
    def __save_dataframe(self, dataset: np.ndarray,
        ...
        """Creates and saves the dataframe version of the dataset.

        Parameters
        ----------
        dataset : ndarray
            The numpy array version of the dataset.

        num_points : int
            The number of points in the dataset.

        sample_freq_seconds : float
            The sampling frequency measured in Hz.

        columns_names : list[str]
            Names of the columns to generate.

        start_timestamp : float
            The timestamp at which to start the observations.
        ...
        timestamps = np.linspace(start, last_timestamp, num_points)
    ...
    def __compute_noise(noise_type: str,
        ...
        Parameters
        ----------
        noise_type : ["Gaussian", "custom"]
            A string representing the type of noise.

        noise_func : Callable
            The function generating the noise in case the noise type is "custom".
    ...
    def __create_process(process_type: str,
        ...
        ----------
        process_type : str
            The type of process between ones allowed.
        ...
        -------
        process : ArmaProcess
            The ARMA process as given by the parameters.
        """
        process = None
